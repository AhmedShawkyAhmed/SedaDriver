...
class _DriverLicenseScreenState extends State<DriverLicenseScreen> {
  ...
  void initState() {
    ...
    if (cubit.checkRegisterResponse?.checkRegisterData?.flags?.driverLicense ==
        1) {
      _enable = false;
      final data = cubit.checkRegisterResponse!.checkRegisterData!.savedData;
      _frontLicense = XFile(
          "${EndPoints.imageBaseUrl}${data?.driverLicense?[0].filename ?? ''}");
      _backLicense = XFile(
          "${EndPoints.imageBaseUrl}${data?.driverLicense?[1].filename ?? ''}");
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: CustomAppBar(title: context.driverLicensePhoto),
                ...
                children: [
                  ...
                  _frontLicense == null
                      ? DefaultTextFormField(
                          ...
                          hintText: context.frontDriverLicense,
                          readOnly: true,
                  ...
                  _backLicense == null
                      ? DefaultTextFormField(
                          controller: TextEditingController(),
                          hintText: context.backDriverLicense,
                          readOnly: true,
                          height: 6.h,
                          borderColor: AppColors.midBlue,
                          onTap: () {
                  ...
                  if (_frontLicense == null || _backLicense == null) {
                    ...
                    final driverImagesUploadRequest = DriverImagesUploadRequest(
                      images: [
                        DriverImage(
                          image: _frontLicense!,
                          imageType: DriverImageType.driverLicenseFront,
                        ),
                        DriverImage(
                          image: _backLicense!,
                          imageType: DriverImageType.driverLicenseBack,
                        ),
                      ],
                    );
