...
class VehicleBrandScreen extends StatefulWidget {
  const VehicleBrandScreen({Key? key}) : super(key: key);
...
class _VehicleBrandScreenState extends State<VehicleBrandScreen> {
  ...
  void initState() {
    super.initState();
    if (AuthCubit.get(context).vehicleTypes.isEmpty) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: CustomAppBar(title: context.vehicleBrand),
        ...
        child: Column(
          children: [
            DefaultTextFormField(
              ...
              hintText: context.vehicleBrandSearch,
              prefix: const Icon(
                Icons.search,
              ),
              suffix: _filled
              ...
              builder: (context, state) {
                ...
                return state is GetVehiclesTypesLoading
                    ...
                    : data.isNotEmpty
                        ? CustomListView(
                            isScrollable: true,
                            data: data.map((e) => "${e.company}").toList(),
                            onTap: (index) {
                              Navigator.pushNamed(
                                context,
                                AppRouterNames.vehicleModel,
                        ...
                        : Column(
                            ...
                            children: [
                              DefaultAppText(
                                text: context.noAvailVehicleBrands,
                                fontSize: 15.sp,
                              ...
                              DefaultAppButton(
                                width: 60.w,
                                text: context.fetchVehicleBrands,
                                onTap: () =>
                                    AuthCubit.get(context).getVehiclesType(),
                              ),
                            ],
