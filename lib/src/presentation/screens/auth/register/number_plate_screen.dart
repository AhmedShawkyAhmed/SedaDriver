...
class NumberPlateScreen extends StatefulWidget {
  const NumberPlateScreen({Key? key}) : super(key: key);
...
class _NumberPlateScreenState extends State<NumberPlateScreen> {
  ...
  void initState() {
    ...
    if (cubit.checkRegisterResponse?.checkRegisterData?.flags
      ...
      _numberPlate = XFile(
          "${EndPoints.imageBaseUrl}${data?.vehiclePlatNumber?[0].filename ?? ''}");
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: CustomAppBar(title: context.numberPlatePhoto),
          ...
          children: [
            Container(
              ...
              child: Column(
                children: [
                  DefaultAppText(
                    text: context.photoOfNumberPlate,
                    fontSize: 13.sp,
                  ...
                  Image.asset(AppAssets.imgNumberPlate),
                  SizedBox(
                    height: 3.h,
                  ),
                  _numberPlate == null
                              ...
                              child: Container(
                                ...
                                decoration: BoxDecoration(
                                  ...
                                  image: DecorationImage(
                                    image: _numberPlate!.path.contains('http')
                                        ? Image.network(_numberPlate!.path)
                                            .image
                                        : Image.file(
                                            File(
                                              _numberPlate!.path,
                  ...
                  if (_numberPlate == null) {
                    ...
                    final driverImagesUploadRequest = DriverImagesUploadRequest(
                      images: [
                        DriverImage(
                          image: _numberPlate!,
                          imageType: DriverImageType.vehiclePlateNumber,
                        ),
                      ],
                    );
